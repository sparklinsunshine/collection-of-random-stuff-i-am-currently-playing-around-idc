...
def print_crossword(list):
    a = Crossword(14, 14, "-", 5000, list)
    a.compute_crossword(2)
    def input_word():
        input_keyword = input('Crossword or Wordsearch?:\n')
        if input_keyword == 'Crossword':
            print(Fore.GREEN, a.display())
            print(a.legend())
            print(len(a.current_word_list), "out of", len(list))
            print(a.debug)
        elif input_keyword == 'Wordsearch':
            print(Fore.BLUE, a.word_find())
            # print(a.legend())
            print(a.word_bank())
            print(len(a.current_word_list), "out of", len(list))
            print(a.debug)
...
def display_solution(crosswrd):
    solution_prompt = input('Display the solution?? (Y/N): ')
    if solution_prompt == 'Y':
        print(Fore.LIGHTGREEN_EX, crosswrd.word_bank())
        print(crosswrd.solution())
    elif solution_prompt == 'N':
        try_again = input('Are you sure? (Y/N): ')
        if try_again == 'N':
            display_solution(crosswrd)
        elif try_again == 'Y':
            pass
        else:
            print(Fore.LIGHTRED_EX, 'Oops, this program ain\'t as intelligent as your pea brain to decipher what you just said. Please check your spelling and try again!')
